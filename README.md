...
- OAuth 로그인(Google·GitHub), 사용자 핸들·프로필
...
- GitHub: https://github.com/settings/developers → OAuth App. 콜백 `…/api/auth/callback/github`
- `.env`에 `AUTH_GOOGLE_ID/SECRET`, `AUTH_GITHUB_ID/SECRET` 입력 후 `AUTH_DEV_LOGIN=0`.
...
3. Vercel 환경변수: `TURSO_DATABASE_URL`, `TURSO_AUTH_TOKEN`, `AUTH_SECRET`,
   `AUTH_GOOGLE_ID/SECRET`, `AUTH_GITHUB_ID/SECRET`. (`AUTH_DEV_LOGIN`은 등록하지 않음)
